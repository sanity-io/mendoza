...
func MaybeApplyPatch(root any, patch Patch) (any, error) {
	return DefaultOptions.MaybeApplyPatch(root, patch)
}
...
//
// This function uses the default options.
func ApplyPatch(root any, patch Patch) any {
	return DefaultOptions.ApplyPatch(root, patch)
}

// Applies a patch to a document. Returns an error if the patch
// cannot be applied (e.g., the document structure doesn't match).
func (options *Options) MaybeApplyPatch(root any, patch Patch) (any, error) {
	if len(patch) == 0 {
		return root, nil
	}
...
// MustApplyPatch applies a patch to a document. It panics if the patch
// cannot be applied (e.g., the document structure doesn't match).
func (options *Options) ApplyPatch(root any, patch Patch) any {
	result, err := options.MaybeApplyPatch(root, patch)
	if err != nil {
		panic(err)
	}
	return result
