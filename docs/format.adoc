...
- `ReturnIntoObjectKeylessPop()`
- `ReturnIntoObjectKeylessPop()`

Note that entering a field remembers the key where it come from, which `ReturnIntoObjectKeyless()` then uses to set it.
...
### `ReturnIntoObjectKeyless` operation

.Parameters
_None_

The `OpReturnIntoObjectKeyless` operation first finds the key that was used to push the current input value (see <<OpPushField,PushField>>),
...
The key is stored together with the value in the input stack so that <<OpReturnIntoObjectKeyless,ReturnIntoObjectKeyless>> can access it later.
...
|5
|<<OpReturnIntoObjectKeyless,ReturnIntoObjectKeyless>>
|Output
|
...
|ReturnIntoObjectKeylessPop
|Composite
|<<OpReturnIntoObjectKeyless,ReturnIntoObjectKeyless>> + <<OpPop,Pop>>
...
|ObjectCopyField
|Composite
|<<OpPushField,PushField>> + <<OpCopy,Copy>> + <<OpReturnIntoObjectKeyless,ReturnIntoObjectKeyless>> + <<OpPop,Pop>>
